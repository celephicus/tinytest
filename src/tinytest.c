...
static void tt_vprintf(tt_pgm_str_t fmt, va_list args) {
    union {
        ...
        tt_pgm_str_t pstr;			/* Fixed string, AVR stores in Flash, non-AVR targets define this type as 'const char*', but is OK to have two union members of same type. */
...
done_format:    informat = 0;
            ...
            if ('%' == c) {		/* We have a format spec cominf, default to base 10. */
                informat = 1;
                base = 10;
            }
            else
...
void ttRunTest(void (*test_func)(void), tt_pgm_str_t filename, int lineno, tt_pgm_str_t desc) {
    if (NULL != strstr(desc, f_ctx.groupstr)) { /* Decide whether to run this test... */
        int exc;
...
static int pause = 0;
static char* tests = (char*)"";		/* Hack, shouldn't assign a string constant to a non-const pointer. But since we do not write the underlying memory it doesn't matter. */
